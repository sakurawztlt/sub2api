export default {
  ...
  admin: {
    ...
    channels: {
      ...
      intervalValidation: {
        ...
        overlap: '区间 #{previousIndex} 和区间 #{currentIndex} 重叠：前一区间上限（{previousMax}）大于当前区间下限（{currentMin}）',
        price: {
          inputPrice: '输入价格',
          outputPrice: '输出价格',
    ...
    riskControl: {
      ...
      preBlockKeyIdleShort: '空闲',
      preBlockKeyFrozenShort: '冻结',
      allGroups: '全部分组',
    ...
    accounts: {
      ...
      bulkEdit: {
        ...
        partialSuccessWithInherited:
          '部分更新成功：成功 {success} 个，失败 {failed} 个；其中 {inherited} 个影子账号仍跟随母账号。',
        failed: '批量更新失败',
      ...
      openai: {
        ...
        wsModePassthroughHint: 'passthrough 与 http_bridge 模式不使用 WS 连接池。',
        ...
        codexFingerprintModeDesc:
          '多人共享同一 OAuth 账号时，将各用户的设备/会话标识收敛为账号级恒定值，减少上游可见的设备数和会话数。默认关闭（原样透传客户端标识），需要时再显式开启；部分账号开启收敛后出现过额度缩水，请按自己的实测结果选择。',
      ...
      headerOverride: {
        ...
        fillTemplate: '填入模板',
        importJson: 'JSON 导入',
        importJsonPlaceholder: '{"accept-language": "zh-CN", "x-relay-token": "..."}',
        importJsonApply: '解析并填入',
        importJsonCancel: '取消',
        importJsonHint: '粘贴扁平 JSON 对象（请求头名到值），解析后将整体替换当前列表。',
        importJsonInvalid: 'JSON 格式不正确：需要“请求头名到字符串值”的扁平对象',
        copyJson: '复制为 JSON',
      ...
      oauth: {
        ...
        grok: {
          ...
          errors: {
            GROK_OAUTH_SESSION_NOT_FOUND: 'OAuth 会话已过期，请重新生成授权链接。',
            GROK_OAUTH_INVALID_STATE: 'OAuth state 无效，请重新生成授权链接后重试。',
            GROK_OAUTH_STATE_REQUIRED: '缺少 OAuth state，请粘贴完整的 callback URL 或查询字符串。',
            GROK_OAUTH_CODE_REQUIRED: '缺少授权码。',
            GROK_OAUTH_NO_REFRESH_TOKEN: 'xAI 未返回 refresh token，请重新授权该账号后重试。',
            GROK_OAUTH_PROXY_NOT_AVAILABLE: '所选代理不可用，请选择其他代理后重试。',
            GROK_OAUTH_PROXY_NOT_FOUND: '所选代理已不存在，请选择其他代理后重试。'
          }
        },
        // Gemini specific
        gemini: {
    ...
    settings: {
      ...
      ollamaCloudUsage: {
        ...
        description: '账号产生模型请求后刷新 Ollama 官方设置页展示的用量；默认关闭。',
        enabled: '启用全局自动刷新',
        enabledHint: '仅刷新已保存浏览器会话且账号自身也开启自动刷新的账号；手动刷新不受影响。',
        intervalMinutes: '最长等待（分钟）',
        intervalHint: '范围 15–1440 分钟。请求持续到来、不断后移安静期时，最晚在此时间强制刷新。',
        debounceMinutes: '末次请求安静期（分钟）',
        debounceHint: '范围 1–60 分钟且小于最长等待。末次模型请求安静满此时长后刷新。',
      ...
      gatewayForwarding: {
        ...
        openaiCodexUserAgentHint: 'OAuth 出站统一使用的完整 Codex User-Agent 指纹。留空则按下方版本号拼出标准 codex_cli_rs 身份；填写后版本段仍由生效版本重建，确保 UA、originator 与 version 不漂移。',
        openaiCodexClientVersion: 'Codex 客户端版本号',
        openaiCodexClientVersionPlaceholder: '留空则跟随自动同步',
        openaiCodexClientVersionHint: 'User-Agent 与 version 头共同对上游声明的版本。留空使用自动同步到的最新稳定版；填写后固定为该版本。',
        openaiCodexVersionAutoSync: '自动同步 Codex 版本号',
        openaiCodexVersionAutoSyncHint: '每 6 小时从官方仓库获取最新稳定版。关闭后仅使用上方固定版本或内置版本。',
      ...
      payment: {
        ...
        quickAmounts: '快捷金额',
        quickAmountsHint: '用英文逗号分隔，在充值页面显示为快捷按钮。',
        quickAmountsFormatError: '快捷金额至少需要一个正数。',
        orderTimeout: '订单超时时间',
        ...
        alipayMobilePrecreateDeepLinkHint: '启用后，移动端官方支付宝订单调用当面付并尝试打开支付宝；失败时显示动态二维码',
        helpText: '帮助文本',
        helpImageUrl: '帮助图片链接',
      ...
      openaiFastPolicy: {
        ...
        modelWhitelist: '目标模型',
        modelWhitelistHint:
          '列表内模型使用“处理方式”，列表外模型使用“其他模型处理方式”。留空时“处理方式”应用于全部模型。支持精确匹配与前缀通配符（如 gpt-5.6*）。',
  ...
  payment: {
    ...
    qr: {
      ...
      cancelOrder: '取消订单',
      amountDue: '应付金额',
      exactAmountWarning: '请严格按照显示金额支付',

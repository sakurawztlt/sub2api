...
describe('AccountUsageCell', () => {
  ...
  it.each([
    { tokens: 0, expected: 0 },
    { tokens: 1_000_000, expected: 50 },
    { tokens: 2_000_000, expected: 100 },
    { tokens: 2_200_000, expected: 100 }
  ])('Grok Free derives its 2M quota from local tokens: $tokens -> $expected%', async ({ tokens, expected }) => {
    getUsage.mockResolvedValue({
      grok_billing: {
        period_type: 'weekly',
        usage_percent: null,

...
describe('OpenAIQuotaResetCell — 外审 F6:影子禁用重置', () => {
  ...
  it('查询期间锁定操作并通过 alert 暴露请求错误', async () => {
    let rejectRequest!: (reason?: unknown) => void
    vi.mocked(refreshOpenAIQuota).mockImplementation(
    ...
    expect(queryButton.attributes('aria-busy')).toBe('true')
    expect(resetButton(wrapper).attributes('disabled')).toBeDefined()

    rejectRequest({ message: 'quota unavailable' })
    await flushPromises()

    const alert = wrapper.get('[role="alert"]')
    expect(alert.text()).toBe('quota unavailable')
    expect(alert.attributes('title')).toBe('quota unavailable')
    expect(queryButton.attributes('aria-busy')).toBe('false')
    wrapper.unmount()
  })

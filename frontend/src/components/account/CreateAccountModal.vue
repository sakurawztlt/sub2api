    ...
    >
      ...
      <div>
        ...
        <div class="mt-2 flex flex-wrap rounded-lg bg-gray-100 p-1 dark:bg-dark-700" data-tour="account-form-platform">
          ...
          <button
            type="button"
            data-testid="account-platform-grok"
            @click="form.platform = 'grok'"
            :class="[
              'flex flex-1 items-center justify-center gap-2 rounded-md px-4 py-2.5 text-sm font-medium transition-all',
              form.platform === 'grok'
                ? 'bg-white text-zinc-800 shadow-sm dark:bg-dark-600 dark:text-zinc-200'
                : 'text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-200'
            ]"
          >
      ...
      <div v-if="form.platform === 'grok'">
        ...
        <div class="mt-2 grid grid-cols-1 gap-3 sm:grid-cols-2" data-tour="account-form-type">
          <button
            type="button"
            data-testid="grok-account-type-oauth"
            @click="accountCategory = 'oauth-based'"
            :class="[
              'flex items-center gap-3 rounded-lg border-2 p-3 text-left transition-all',
              accountCategory === 'oauth-based'
      ...
      <!-- Grok OAuth custom forwarding endpoint; authorization and refresh stay official. -->
      <div
      ...
      >
        <div class="mb-3 flex items-center justify-between gap-4">
          ...
          <button
            ...
            :class="[
              'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2',
      ...
      <!-- Grok OAuth safe request-header overrides. -->
      <div
        v-if="form.platform === 'grok' && isOAuthFlow"
        class="border-t border-gray-200 pt-4 dark:border-dark-600"
      >
        <div class="mb-3 flex items-center justify-between gap-4">
          ...
          <button
            ...
            data-testid="grok-oauth-header-override-toggle"
            @click="headerOverrideEnabled = !headerOverrideEnabled"
            :class="[
              'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2',
              headerOverrideEnabled ? 'bg-primary-600' : 'bg-gray-200 dark:bg-dark-600'
      ...
      <!-- OpenAI WS Mode（off/ctx_pool/passthrough/http_bridge） -->
      <div
      ...
      <!-- OpenAI API 长上下文计费开关；分组已统一开启时不再重复展示账号级开关。 -->
      <div
...
// 批量导入 Codex 会话时，未触碰开关就不覆盖各会话已有的账号级设置。
...
const handleSubmit = async () => {
  ...
  if (isOAuthFlow.value) {
    ...
    if (form.platform === 'grok' && !validateGrokOAuthUpstreamConfig()) {
      return
    }
    const canContinue = await ensureAntigravityMixedChannelConfirmed(async () => {
      step.value = 2
...
// Grok Web SSO 批量转换和创建；原始 SSO 仅发送给专用导入端点。
const handleGrokImportSSO = async (ssoInput: string) => {
  const ssoTokens = ssoInput
    .split('\n')
    .map((token) => token.trim())
    .filter(Boolean)
  if (ssoTokens.length === 0 || !validateGrokOAuthUpstreamConfig()) return
  ...
  try {
    ...
    if (successCount > 0 && failedCount === 0) {
      ...
      grokOAuth.error.value = result.failed
        .map((item) => `#${item.index}: ${item.error || 'Conversion failed'}`)
        .join('\n')
      emit('created')
    } else {
      grokOAuth.error.value =
...
const handleGrokValidateRT = async (refreshTokenInput: string) => {
  if (!refreshTokenInput.trim() || !validateGrokOAuthUpstreamConfig()) return
 ...
 * Password is only sent to the authorization endpoint and is never stored.
 */
const handleGrokAuthorizePassword = async (emailPasswordInput: string) => {
  if (!emailPasswordInput.trim() || !validateGrokOAuthUpstreamConfig()) return

  const lines = emailPasswordInput
    .split('\n')
    // Preserve the password portion byte-for-byte; trim only determines blank lines.
...
const handleGrokExchange = async (authCode: string) => {
  if (!authCode.trim() || !grokOAuth.sessionId.value || !validateGrokOAuthUpstreamConfig()) return

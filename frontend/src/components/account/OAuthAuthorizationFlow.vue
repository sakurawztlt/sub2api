<template>
  ...
  >
      ...
      <div class="flex-1">
        ...
        <!-- Grok email + password is converted to OAuth credentials; password is never stored. -->
...
interface Props {
  ...
  /** Prefill only an email or "email----"; never pass a stored password. */
...
const showLocalCallbackNotice = computed(
  () => props.platform === 'openai' || props.platform === 'grok'
)
...
watch(
  ...
  (newVal) => {
    if (newVal && !emailPasswordInput.value.trim()) emailPasswordInput.value = newVal
  }
)
...
// Auto-extract code and state from callback URL/query text.

...
describe('BulkEditUserModal', () => {
  ...
  it('submits only the enabled RPM field and preserves zero as unlimited', async () => {
    ...
    expect(batchUpdateLimits).toHaveBeenCalledWith({ user_ids: [4, 7], all: false, rpm_limit: 0 })
    expect(confirm).toHaveBeenCalledWith(expect.stringContaining('admin.users.bulkLimits.rpmUnlimitedValue'))
  ...
  it('omits disabled fields from the request', async () => {
    ...
    expect(batchUpdateLimits).toHaveBeenCalledWith({ user_ids: [4, 7], all: false, concurrency: 9 })
  })

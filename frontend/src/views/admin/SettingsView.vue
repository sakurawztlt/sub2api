...
// 2026-05-05 fork override: upstream PR #2120 默认 5s 在 ChatGPT Pro 池上太
// 激进 (账号被 429 后 5s 就重试 → 反复撞同 dead 账号 → 加速封号). 保留 5min
// 安全默认, 仅放开 admin UI 让 ops 按需调.
...
async function saveSettings() {
  ...
  try {
    ...
    // codex upstream PR#2432 cherry-pick — payment_quick_amounts validation
    // only. Skip the login_agreement_* hunks from the same upstream block
    // (separate feature, references undefined helpers normalizeLoginAgreement
    // DocumentsForSave / findDuplicateLoginAgreementDocumentId in local fork).

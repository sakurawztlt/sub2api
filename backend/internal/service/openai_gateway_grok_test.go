...
func TestAccountTestServiceGrokOAuthPaymentRequiredTemporarilyUnschedulesAccount(t *testing.T) {
	...
	err := svc.testGrokAccountConnection(c, account, "grok", "", "", AccountTestOptions{})
...
func TestForwardAsAnthropicForGrokUsesXAIResponses(t *testing.T) {
	...
	upstream := &httpUpstreamRecorder{resp: grokMessagesSSECompletedResponse("resp_grok_messages", 5, 3)}
	svc := &OpenAIGatewayService{
		httpUpstream:      upstream,
		grokTokenProvider: NewGrokTokenProvider(repo, nil),
		accountRepo:       repo,

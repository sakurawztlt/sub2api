...
//
// 当前路由策略（基于账号覆盖模式/探测标记/base_url，详见
// openai_compat.ShouldUseResponsesAPIForBaseURL）：
//   - APIKey 账号 + 强制、探测确认或已知 base_url 不支持 Responses →
//     走 forwardAsRawChatCompletions
...
func (s *OpenAIGatewayService) ForwardAsChatCompletions(
	...
	// Some clients send a Responses-shaped body to the Chat Completions URL.
	// Detect it before adaptive routing so it is never forwarded unchanged to a
...
func (s *OpenAIGatewayService) handleChatBufferedStreamingResponse(
	...
	finalResponse, usage, acc, err := s.readOpenAICompatBufferedTerminal(c.Request.Context(), resp, c, "openai chat_completions buffered", requestID, 0)
	if err != nil {
...
func (s *OpenAIGatewayService) handleChatStreamingResponse(
	...
	processDataLine := func(payload string) bool {
		...
		if strings.TrimSpace(event.Type) == "response.failed" || strings.TrimSpace(event.Type) == "error" {
			...
			if hit, code, msg := detectOpenAICyberPolicy(payloadBytes); hit {
				MarkOpsCyberPolicy(c, CyberPolicyMark{
					Code: code, Message: msg, Body: truncateString(string(payloadBytes), 4096),
					UpstreamStatus: http.StatusOK, UpstreamInTok: usage.InputTokens, UpstreamOutTok: usage.OutputTokens,
				})
	...
	// codex round23 / upstream cc5328c49 fu40: frame-aware variant of
	// processDataLine — patches `type` into payloads that arrived via
	// `event: <name>` form (no type field in data). [DONE] is treated as
	// a terminal-via-sentinel by the caller; this helper only forwards
	// real data payloads.
	...
	for {
		...
		case ev, ok := <-events:
			if !ok {
				// codex round23 fu40: flush parser before declaring missing
				// terminal — the final event may have arrived without a

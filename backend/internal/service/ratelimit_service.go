...
const (
	...
	// 池上太激进 (账号被 429 后 5s 就重试 → 反复撞同 dead 账号 → 加速封号).
	// 保留 5min 安全默认 (跟 PR 之前的硬编码一致), 仅放开 admin UI 让 ops 调.
...
func (s *RateLimitService) HandleUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte, requestedModel ...string) (shouldDisable bool) {
	...
	case 401:
		// Spark 影子不持有独立凭据；401 的缓存失效、刷新判断及状态变更都必须落到母账号。
		authAccount := account
		...
		if authAccount.Type == AccountTypeOAuth {
			...
			//
			// Do not write account.Credentials back from this request-start
			// snapshot. Another worker may have refreshed refresh_token just
			// before this 401 is handled; writing the stale snapshot would roll
			// the DB token back and turn a recoverable refresh race into a dead
			// account. InvalidateToken + temporary unscheduling is enough: after
			// cooldown, token_provider re-reads DB and refreshes under its lock.
...
func (s *RateLimitService) handle429(ctx context.Context, account *Account, headers http.Header, responseBody []byte) {
	...
	// window. Persisting a reset on the first 429 would make that retry ineligible.
	...
	// Spark shadow quota is sourced only from the bengalfox usage probe; global
	// Codex response headers must never overwrite its independent snapshot.
	if account.IsShadow() {
	...
	if resetTimestamp == "" {
		...
		// Anthropic 平台没有重置时间的 429 也需短暂冷却，否则每次请求都会
		// 反复撞同一批账号并耗尽 failover 预算；该冷却可由管理设置调整或关闭。
...
func (s *RateLimitService) HandleOpenAICodexSparkRateLimit(ctx context.Context, account *Account, requestedModel string, statusCode int, headers http.Header, responseBody []byte) bool {
	...
	if resetAt == nil || !resetAt.After(now) {
		// The fork's account-wide fallback intentionally remains five minutes to
		// protect OAuth credentials from repeated retries. Spark is model-scoped,
		// so retain upstream's short transient fallback without parking the other
		// models on the credential.
		reset := now.Add(openAIOAuth429FallbackCooldown)
		resetAt = &reset
	}

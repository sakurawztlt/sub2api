...
func TestCreateOpenAICodexUsageProbePayloadUsesCompatibleModel(t *testing.T) {
	t.Parallel()

	payload := createOpenAICodexUsageProbePayload()
	if got := payload["model"]; got != "codex-auto-review" {
		t.Fatalf("model = %v, want codex-auto-review", got)
	}
	if got := payload["store"]; got != false {
		t.Fatalf("store = %v, want false for OAuth probe", got)
	}
}

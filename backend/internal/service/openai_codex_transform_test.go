...
func TestApplyCodexOAuthTransform_NormalizesIsolatedLegacyReferenceAcrossTurns(t *testing.T) {
	...
	// Stored Responses may retain isolated references across turns. The
	// store=false OAuth path is intentionally covered above and must continue
	// dropping them to avoid ChatGPT's non-persisted-item 404.
	applyCodexOAuthTransform(reqBody, false, false, true)
...
func TestApplyCodexOAuthTransform_PreservesNativeCallIDsWhenRequestedButStripsReplayItemIDs(t *testing.T) {
	reqBody := map[string]any{
		"model": "gpt-5.6-sol",
		"input": []any{
	...
	require.True(t, ok)
	require.NotContains(t, first, "id", "store=false OAuth forwarding must not replay server item ids")
	require.Equal(t, "call_custom", first["call_id"])
	require.Equal(t, "call_custom", second["call_id"])
	require.NotContains(t, third, "id", "store=false OAuth forwarding must not replay server item ids")
...
func TestApplyCodexOAuthTransform_ConvertsToolRoleMessageToFunctionCallOutput(t *testing.T) {
	...
	applyCodexOAuthTransform(reqBody, true, false, false)
...
func TestApplyCodexOAuthTransform_PreservesKnownAdditionalToolChoice(t *testing.T) {
	reqBody := map[string]any{
		"model": "gpt-5.6-sol",
		"input": []any{
			map[string]any{
	...
	require.Equal(t, map[string]any{"type": "custom"}, reqBody["tool_choice"])
}

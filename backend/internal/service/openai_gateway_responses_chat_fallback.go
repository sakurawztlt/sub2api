...
func (s *OpenAIGatewayService) forwardResponsesViaRawChatCompletions(
	...
	// Build and send through the shared Chat Completions pipeline so CN
	// providers use the protocol-family credential and the same proxy/header/
	// error semantics as /v1/chat/completions and /v1/messages fallbacks.
...
type responsesReasoningGatewayCache interface {
...
func (s *OpenAIGatewayService) reasoningContentByID(itemID string) string {
	...
	defer cancel()
	cache, ok := s.cache.(responsesReasoningGatewayCache)
	if !ok {
		return ""
	}

...
const (
	...
	// generation away from Grok accounts that are explicitly disabled or whose
	// billing entitlement probe was forbidden. Existing video lookups do not
	// require this capability, so submitted requests remain queryable.
	...
	// API-key 上游使用自动探测标记判定；OAuth 保留既有支持语义。
...
// IsCredentialUsableForShadow reports whether this parent account's shared
// OAuth credential and transport are usable by a linked spark shadow. Global
// quota/overload windows and the parent's manual schedulable flag are
// intentionally excluded because spark has an independent quota dimension.
...
// IsPoolModeRetryableStatus 在账号上下文中判断状态码是否应触发同账号重试。
...
// QuotaDimensionOrDefault returns the persisted quota dimension, treating
// legacy rows with an empty value as the global dimension.
...
// GetGrokBaseURL selects the forwarding upstream for Grok text traffic.
// OAuth credential lifecycle requests remain pinned to the official auth
// endpoints; only an explicitly configured custom host redirects inference.
...
func (a *Account) SupportsOpenAIEndpointCapability(capability OpenAIEndpointCapability) bool {
	...
	if a.IsGrok() {
		...
		case OpenAIEndpointCapabilityGrokMediaGeneration:
			...
			// path can probe billing before forwarding. Forwarding itself fails
			// closed without positive paid-entitlement evidence.
	...
	case OpenAIEndpointCapabilityLive:
		return a.Platform == PlatformOpenAI &&
			...
			!strings.EqualFold(strings.TrimSpace(a.GetCredential(openAIAuthModeCredentialKey)), "agentIdentity")
	case OpenAIEndpointCapabilityResponses:
		// 已探测确认不支持 /v1/responses 的 APIKey 上游必须排除，
		// 否则 forward 会降级为 Chat Completions 而无法完成 Responses 生图。

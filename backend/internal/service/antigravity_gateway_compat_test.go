...
func antigravityCompatSuccessResponse() *http.Response {
	body := `data: {"response":{"responseId":"resp_3757","candidates":[{"content":{"parts":[{"text":"ok"}]},"finishReason":"STOP"}],"usageMetadata":{"promptTokenCount":8,"cachedContentTokenCount":3,"candidatesTokenCount":3}}}` + "\n\n"
...
func TestAntigravityCompatOAuthUsesNativeTokenAndRoute(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Equal(t, "request-3757", recorder.Header().Get("X-Request-Id"))
...
func TestAntigravityCompatRejectsUnsupportedAccountType(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Contains(t, recorder.Body.String(), "The selected account cannot handle this request.")
			require.NotContains(t, strings.ToLower(recorder.Body.String()), "antigravity")
			require.NotContains(t, strings.ToLower(recorder.Body.String()), "oauth")
		})
	}
}
...
func TestAntigravityCompatFirstEventTimeoutTriggersFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	svc := newAntigravityCompatService(
		config.GatewayConfig{MaxLineSize: defaultMaxLineSize, FirstMeaningfulEventTimeoutSeconds: 1},
		nil,
	)
	c, recorder := newAntigravityCompatContext(http.MethodPost, "/v1/chat/completions", nil)
	...
	heartbeat := time.NewTicker(150 * time.Millisecond)
	...
	for {
		...
		case got := <-done:
			...
			return
		case <-heartbeat.C:
			_, _ = io.WriteString(writer, `data: {"response":{"responseId":"resp_heartbeat","usageMetadata":{"promptTokenCount":8}}}`+"\n\n")
		case <-deadline:
			_ = writer.Close()
			_ = reader.Close()
			t.Fatal("usage-only heartbeats bypassed FirstMeaningfulEventTimeoutSeconds")
		}
	}
}

func TestAntigravityCompatBufferedFirstMeaningfulTimeoutIgnoresUsageHeartbeats(t *testing.T) {
	svc := newAntigravityCompatService(
		config.GatewayConfig{MaxLineSize: defaultMaxLineSize, BufferedFirstMeaningfulTimeout: 1},
		nil,
	)
	reader, writer := io.Pipe()
	resp := &http.Response{StatusCode: http.StatusOK, Header: http.Header{}, Body: reader}
	...
	go func() {
		body, result, err := svc.collectClaudeStreamResponse(resp, time.Now(), "gemini-3.1-pro-high")
		done <- outcome{body: body, result: result, err: err}
	}()
	...
	for {
		...
		case got := <-done:
			...
			var failoverErr *UpstreamFailoverError
			require.ErrorAs(t, got.err, &failoverErr)
			require.Equal(t, "buffered_first_meaningful_timeout", failoverErr.Reason)
			_ = writer.Close()
			_ = reader.Close()
			return
		case <-heartbeat.C:
		...
		case <-deadline:
			_ = writer.Close()
			_ = reader.Close()
			t.Fatal("usage-only heartbeats bypassed BufferedFirstMeaningfulTimeout")
		}
	}
}

func TestAntigravityCompatBufferedTotalTimeoutSynthesizesMeaningfulResponse(t *testing.T) {
	svc := newAntigravityCompatService(
		config.GatewayConfig{MaxLineSize: defaultMaxLineSize, BufferedTotalTimeout: 1},
		nil,
	)
	reader, writer := io.Pipe()
	resp := &http.Response{StatusCode: http.StatusOK, Header: http.Header{}, Body: reader}
	...
	_, err := io.WriteString(
		writer,
		`data: {"response":{"responseId":"resp_partial","candidates":[{"content":{"parts":[{"text":"partial"}]}}],"usageMetadata":{"promptTokenCount":8,"candidatesTokenCount":1}}}`+"\n\n",
	)
	require.NoError(t, err)
	...
	case got := <-done:
		...
		require.Contains(t, string(got.body), "partial")
		require.Equal(t, 8, got.result.usage.InputTokens)
		require.Equal(t, 1, got.result.usage.OutputTokens)
	case <-time.After(2 * time.Second):
		_ = writer.Close()
		_ = reader.Close()
		t.Fatal("buffered meaningful response ignored BufferedTotalTimeout")
	}
	_ = writer.Close()
	_ = reader.Close()
}

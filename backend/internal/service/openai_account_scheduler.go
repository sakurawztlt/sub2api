...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	account = s.service.recheckSelectedOpenAICompatibleAccountFromDB(ctx, account, req.Platform, req.RequestedModel, req.RequireCompact, req.RequiredCapability)
...
func (s *defaultOpenAIAccountScheduler) tryAcquireOpenAISelectionOrder(
	...
	for i := 0; i < len(selectionOrder); i++ {
		...
		fresh := s.service.resolveFreshSchedulableOpenAICompatibleAccount(ctx, candidate.account, req.Platform, req.RequestedModel, false, req.RequiredCapability)
		if fresh == nil || !s.isAccountTransportCompatible(fresh, req.RequiredTransport) || !s.isAccountRequestCompatible(ctx, fresh, req) {
			continue
		}
		fresh = s.service.recheckSelectedOpenAICompatibleAccountFromDB(ctx, fresh, req.Platform, req.RequestedModel, false, req.RequiredCapability)
		if fresh == nil || !s.isAccountTransportCompatible(fresh, req.RequiredTransport) || !s.isAccountRequestCompatible(ctx, fresh, req) {
			continue
		}
...
// openAISelectionFilterStats captures why the initial load-balance pool shrank.
// It is only rendered on failure, so successful scheduling remains allocation-light.
...
func (s *defaultOpenAIAccountScheduler) selectByLoadBalance(
	...
	accounts, err := s.service.listSchedulableAccountsByPlatform(ctx, req.GroupID, req.Platform)
	if err != nil {
		...
		if len(subscriptionAccounts) > 0 {
			...
			if len(regularAccounts) > 0 {
				...
				if regularAttempt.err == nil {
					result, candidateCount, topK, loadSkew, fallbackErr = s.finishLoadBalanceSelectionFallback(ctx, req, regularAttempt, filterStats)
				...
				subResult, subCandidateCount, subTopK, subLoadSkew, subErr := s.finishLoadBalanceSelectionFallback(ctx, req, attempt, filterStats)
			...
			return s.finishLoadBalanceSelectionFallback(ctx, req, attempt, filterStats)
		}
	}
...
func (s *OpenAIGatewayService) selectAccountWithScheduler(
	...
	selection, decision, err := s.selectAccountWithSchedulerOnce(ctx, groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, requiredCapability, requiredImageCapability, requireCompact, platform, previousResponseCanMove)
	...
	return s.selectAccountWithSchedulerOnce(withOpenAIProxyStreamQuarantineBypass(ctx), groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, requiredCapability, requiredImageCapability, requireCompact, platform, previousResponseCanMove)
}
...
func (s *OpenAIGatewayService) selectAccountWithSchedulerOnce(
	...
	if scheduler == nil {
		...
		if requiredTransport == OpenAIUpstreamTransportAny || requiredTransport == OpenAIUpstreamTransportHTTPSSE {
			...
			for {
				selection, err := s.selectOpenAICompatibleAccountWithLoadAwareness(ctx, platform, groupID, sessionHash, requestedModel, effectiveExcludedIDs, requireCompact, requiredCapability)
				if err != nil {
					return nil, decision, err
				}
				if selection == nil || selection.Account == nil {

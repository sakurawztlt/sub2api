...
const (
	// AntigravityCredentialRejectedClientMessage is retained for internal ops
	// diagnostics only. Client-facing handlers deliberately replace it with a
	// provider-neutral availability message.
...
func (s *AntigravityGatewayService) validateAntigravityCompatAccount(c *gin.Context, account *Account) error {
	...
	return s.writeAntigravityCompatError(
		...
		"The selected account cannot handle this request.",
	)
}
...
func (s *AntigravityGatewayService) prepareAntigravityCompatCall(
	...
	if s.tokenProvider == nil {
		return nil, s.writeAntigravityCompatError(c, http.StatusBadGateway, "api_error", antigravityCompatTemporaryUnavailableMessage)
	}
	accessToken, err := s.tokenProvider.GetAccessToken(ctx, account)
	if err != nil {
		logger.LegacyPrintf(
			"service.antigravity_gateway",
			"compat token acquisition failed: account_id=%d account_name=%q error=%v",
			account.ID,
			account.Name,
			err,
		)
		return nil, &UpstreamFailoverError{
			...
			ResponseBody: []byte(`{"error":{"type":"api_error","message":"The service is temporarily unavailable. Please retry."},"type":"error"}`),
		}
	}
	...
	if err != nil {
		logger.LegacyPrintf(
			"service.antigravity_gateway",
			"compat account configuration error: account_id=%d account_name=%q error=%v",
			account.ID,
			account.Name,
			err,
		)
		return nil, &UpstreamFailoverError{
			...
			Reason:            "account_configuration_error",
		}
	}
...
func (s *AntigravityGatewayService) writeMappedAntigravityCompatError(
	...
	c.JSON(mapUpstreamStatusCode(upstreamStatus), gin.H{
		"error": gin.H{
			"message": antigravityCompatTemporaryUnavailableMessage,
			"type":    "api_error",
			"param":   nil,
			"code":    nil,
		},

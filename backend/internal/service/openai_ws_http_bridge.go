...
// shouldBridgeOpenAIWSPassthroughFirstMessage applies the large-payload HTTP
// bridge only to an initial response.create frame. A previous_response_id turn
// must remain on the WebSocket path so upstream conversation state is retained.
func (s *OpenAIGatewayService) shouldBridgeOpenAIWSPassthroughFirstMessage(account *Account, payload []byte) bool {
	if account != nil && account.Platform == PlatformGrok {
		return true
	}
	if !s.openAIWSHTTPBridgeEnabled() || int64(len(payload)) < s.openAIWSHTTPBridgeThresholdBytes() || !json.Valid(payload) {
		return false
	}
...
func (s *OpenAIGatewayService) proxyOpenAIWSHTTPBridgeTurn(
	...
	for {
		...
		if account.Platform == PlatformGrok {
			...
			s.handleGrokAccountUpstreamError(
				withGrokTeamRateLimitModel(ctx, resolveGrokWSUpstreamModel(account, body, originalModel)),
				account,
				resp.StatusCode,
				resp.Header,
				respBody,
	...
	for scanner.Scan() {
		...
		if eventType == "error" || eventType == "response.failed" {
			...
			if !wroteDownstream && !openAIUsageHasTokens(&usage) && shouldFailover && (turn == 1 || statusCode == http.StatusTooManyRequests) {
				if account.Platform == PlatformGrok {
...
func resolveGrokWSUpstreamModel(account *Account, body []byte, originalModel string) string {
	...
	// Shared ingress has already applied channel/account mappings when the body
	// model differs from the client-facing value.

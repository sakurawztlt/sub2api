...
func Relay(
	...
	// upstream disconnect. Leave the client connection open so the adapter can
	// emit the precise lease/request close code before relay cancellation.
	if ctx.Err() != nil {
...
func observeRelayTurnResponseModel(turn *relayTurnTiming, model string, terminal bool) {
	...
	model = upstreammodel.Normalize(model)
	if model == "" {
		return
	}
...
func parseUsageAndAccumulate(
	...
	// Responses websocket usage frames are snapshots for the current turn, not
	// deltas. Keep the best per-turn snapshot and add it to the connection total
	// exactly once when a terminal event settles the turn. Summing every frame
	// would overbill streams that report both in-progress and terminal usage.
	if isTerminalEvent(strings.TrimSpace(eventType)) {
		// A terminal snapshot is authoritative when it carries usage. If it is
		// empty, retain the last non-terminal snapshot as a billing fallback.
...
// openAICacheCreationTokensFromUsage normalizes cache-write aliases emitted by
// Responses- and Chat-compatible websocket providers. Prefer nested detail
// fields because they are the canonical OpenAI usage shape.

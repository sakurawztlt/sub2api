...
func TestParseUsageAndEnrichCoverage(t *testing.T) {
	...
	parseUsageAndAccumulate(state, []byte(`{"type":"response.completed","response":{"usage":{"input_tokens":2,"output_tokens":1,"input_tokens_details":{"cached_tokens":1,"image_tokens":2},"cache_creation_input_tokens":4,"output_tokens_details":{"image_tokens":3}}}}`), "response.completed", nil)
	...
	require.Equal(t, 2, state.usage.ImageInputTokens)
...
func TestObserveRelayTurnResponseModelBoundsUntrustedUnicodeModel(t *testing.T) {
	t.Parallel()

	turn := &relayTurnTiming{}
	observeRelayTurnResponseModel(turn, "  "+strings.Repeat("模", 201)+"  ", true)
	require.Len(t, []rune(relayTurnResponseModel(turn)), 200)
}

...
func (s *OpenAIGatewayService) markOpenAIOAuth429RateLimited(ctx context.Context, account *Account, headers http.Header, responseBody []byte) {
	...
	// Spark has an independent quota dimension. A global /responses 429 from
	// its parent credential must not park the shadow until the parent's reset.
	if account.IsShadow() {
		return
	}
...
func (s *OpenAIGatewayService) openAIAccountRuntimeBlockLock(accountID int64) *sync.Mutex {
	...
	if !ok {
		panic("openai account runtime block lock has unexpected value type")
	}
	return lock
}

...
// gitHubRecentReleaseClient is the optional list capability used by the
// fallback path. Keeping it separate preserves the relay's narrower updater
// interface while the concrete GitHub client still provides complete sync.
type gitHubRecentReleaseClient interface {
...
func (s *OpenAICodexVersionSyncService) fetchLatestStableVersion(ctx context.Context) string {
	...
	recentClient, ok := s.githubClient.(gitHubRecentReleaseClient)
	if !ok {
		slog.Warn("openai_codex_version_sync_list_fallback_unavailable")
		return ""
	}
	releases, err := recentClient.FetchRecentReleases(ctx, openAICodexVersionSyncRepo, openAICodexVersionSyncPerPage)
	if err != nil {

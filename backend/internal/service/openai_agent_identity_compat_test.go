...
func TestOpenAIAgentIdentityPassthroughKeepsSessionAndPromptCacheHeaders(t *testing.T) {
	...
	// The local relay deliberately aligns session_id to conversation_id when
	// both are present. This prevents a client-reused session_id from carrying
	// upstream state across otherwise independent conversations.
	require.Equal(t, isolateOpenAIUpstreamSessionID(0, account, "client-conversation"), req.Header.Get("session_id"))

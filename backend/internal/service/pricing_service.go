...
// warnDroppedLongContextLadders prevents a catalog refresh from silently
// removing an existing data-driven long-context pricing ladder.
...
func (s *PricingService) buildModelLookupCandidates(modelLower string) []string {
	...
	// Prefer a future tier-specific Gemini price when present; today all 3.6
	// Flash thinking tiers fall back to the base model's published rate.
...
func (s *PricingService) ListModelNamesByProvider(provider string) []string {
	...
	for name, pricing := range s.pricingData {
		if strings.ToLower(strings.TrimSpace(pricing.LiteLLMProvider)) == provider {

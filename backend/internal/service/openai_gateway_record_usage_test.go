...
func TestOpenAIGatewayServiceRecordUsage_WSModePrefersUpstreamRequestIDOverClientRequestID(t *testing.T) {
	...
	ctx := context.WithValue(context.Background(), ctxkey.ClientRequestID, "openai-ws-client-connection-123")
	err := svc.RecordUsage(ctx, &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			RequestID:    "resp_openai_ws_upstream_turn_456",
			OpenAIWSMode: true,
			Usage: OpenAIUsage{
				InputTokens:  8,
				OutputTokens: 4,
	...
	require.NoError(t, err)
	require.NotNil(t, billingRepo.lastCmd)
	require.Equal(t, "resp_openai_ws_upstream_turn_456", billingRepo.lastCmd.RequestID)
	require.NotNil(t, usageRepo.lastLog)
	require.Equal(t, "resp_openai_ws_upstream_turn_456", usageRepo.lastLog.RequestID)
}
...
func TestOpenAIGatewayServiceRecordUsage_Gpt54LongContextBillingEnabledPerAccount(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotNil(t, usageRepo.lastLog)
	require.InDelta(t, 300000*2.5e-6*2.0, usageRepo.lastLog.InputCost, 1e-10)
	require.InDelta(t, 2000*15e-6*1.5, usageRepo.lastLog.OutputCost, 1e-10)
...
func TestOpenAIGatewayServiceRecordUsage_GroupOrAccountLongContextAllows(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "group on account off", groupEnabled: true},
		{name: "group off account on", accountEnabled: true},
		{name: "group on account on", groupEnabled: true, accountEnabled: true},
	}
	for i, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
				Result: &OpenAIForwardResult{
					RequestID: "resp_long_context_or_gate",
					Usage:     tokens,
					Model:     "gpt-5.4-2026-03-05",
					Duration:  time.Second,
				},
				APIKey: openAIRecordUsageAPIKeyWithGroup(svc, int64(1020+i), tt.groupEnabled),
				User:   &User{ID: int64(2020 + i)},
				Account: &Account{
					ID:       int64(3020 + i),
					Platform: PlatformOpenAI,
					Extra:    map[string]any{openAILongContextBillingEnabledKey: tt.accountEnabled},
				},
			})
			require.NoError(t, err)
			require.True(t, usageRepo.lastLog.LongContextBillingApplied)

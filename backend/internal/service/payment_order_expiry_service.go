...
const (
	// paymentOrderExpiryLeaderLockKey gates the periodic expiry sweep so only one
	// instance updates payment orders per cycle.
	paymentOrderExpiryLeaderLockKey = "payment:order:expiry:leader"
	// paymentOrderExpiryLeaderLockTTL must exceed expiryCheckTimeout so the lock
...
// instance for the periodic expiry sweep. When both are nil the job
...
func (s *PaymentOrderExpiryService) runOnce() {
	// Multi-instance guard: only the leader expires orders per cycle, avoiding
	// N× update races.

...
func (p *openAIWSConnPool) effectiveMaxConnsByAccount(account *Account) int {
	...
	if p.modeRouterV2Enabled() {
		...
		// Router V2 treats the account's concurrency as the authoritative
		// scheduling limit. The legacy pool cap and OAuth/API-key factors belong
		// only to the old router path.

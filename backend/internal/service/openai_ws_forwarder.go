...
func resolveOpenAIWSTurnUpstreamModel(account *Account, payload []byte, originalModel string) string {
	if payloadModel := strings.TrimSpace(gjson.GetBytes(payload, "model").String()); payloadModel != "" {
		return payloadModel
	}
...
func parseOpenAIWSResponseUsageFromCompletedEvent(message []byte, usage *OpenAIUsage) {
	...
	// FromJSONBytes which understands both Responses-shape and Chat-
	// Completions-shape usage. WS path was zeroing out tokens when upstream
	// emitted prompt_tokens/completion_tokens.
...
func (s *OpenAIGatewayService) buildOpenAIWSHeaders(
	...
	if err != nil {
		return nil, openAIWSSessionHeaderResolution{}, fmt.Errorf("build openai authentication headers: %w", err)
	}
	...
	// A real Codex session carries the same beta-feature identity on HTTP and
	// WebSocket handshakes. Preserve explicit client declarations and inject
	// the OAuth default only when the client omitted it.
...
func (s *OpenAIGatewayService) buildOpenAIWSCreatePayload(reqBody map[string]any, account *Account) map[string]any {
	...
	if account != nil && account.UsesOpenAICodexProtocol() && !s.isOpenAIWSStoreRecoveryAllowed(account) && !account.IsOpenAIStoreEnabled() {
		payload["store"] = false
	}
	return payload
}
...
func (s *OpenAIGatewayService) forwardOpenAIWSV2(
	...
	wsHeaders, sessionResolution, buildHeadersErr := s.buildOpenAIWSHeaders(
		ctx,
		c,
		account,
		token,
	...
	if buildHeadersErr != nil {
		return nil, fmt.Errorf("build ws headers: %w", buildHeadersErr)
	}
...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	// The Gin context is reused across failover attempts. Reset attempt-scoped
	// aliases and overwrite the staged OAuth identity before shaping the body.
	...
	runHTTPBridge := func(currentBridgePayload openAIWSClientPayload, bridgeReplayInput []json.RawMessage, bridgeReplayInputExists bool, firstBridgeTurn int, skipFirstTurnHooks bool) error {
		...
		bridgeAccountFailoverInput := cloneOpenAIWSRawMessages(bridgeReplayInput)
		bridgeAccountFailoverInputExists := bridgeReplayInputExists
	...
	wsHeaders, _, buildHeadersErr := s.buildOpenAIWSHeaders(
		ctx,
		c,
		account,
		token,
	...
	if buildHeadersErr != nil {
		return fmt.Errorf("build ws headers: %w", buildHeadersErr)
	}
	proxyURL := account.ProxyURL()
	...
	sendAndRelay := func(turn int, lease *openAIWSConnLease, payload []byte, payloadBytes int, originalModel string, imageBillingModel string, imageSizeTier string, imageInputSize string) (*OpenAIForwardResult, error) {
		...
		var mappedModelBytes []byte
		if originalModel != "" {
			mappedModel = resolveOpenAIWSTurnUpstreamModel(account, payload, originalModel)
		...
		for {
			...
			if eventType == "error" {
				...
				// Only the handler's orchestrated path (non-nil hooks) can safely
				// select another account. Direct callers must receive the sanitized
				// error event so Codex can apply its own retry policy.
				if hooks != nil && !wroteDownstream && isOpenAIUpstreamCapacityShedEvent(upstreamMessage) {
					lease.MarkBroken()
	...
	for {
		...
		if nextPayload.promptCacheKey != "" {
			...
			updatedHeaders, _, updateHeadersErr := s.buildOpenAIWSHeaders(
				ctx,
				c,
				account,
				token,
			...
			if updateHeadersErr != nil {
				logOpenAIWSModeInfo("ingress_ws_update_headers_failed account_id=%d err=%v", account.ID, updateHeadersErr)
			} else {
				baseAcquireReq.Headers = updatedHeaders
			}
		}

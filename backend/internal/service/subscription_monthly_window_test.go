...
func (r *monthlyResetUserSubRepo) ResetMonthlyUsage(_ context.Context, _ int64, resetAt time.Time) error {
	r.resetCalled = true
	r.resetAt = resetAt
	return nil
}
...
func TestThirtyDaySubscriptionDoesNotResetMonthlyQuotaBeforeExpiry(t *testing.T) {
	...
	require.False(t, sub.NeedsMonthlyResetAt(expiresAt.Add(-time.Second)))
	require.True(t, sub.NeedsMonthlyResetAt(expiresAt))
	require.False(t, sub.canAutomaticallyResetMonthlyAt(expiresAt))
	require.Equal(t, expiresAt, *sub.MonthlyResetTime())
}

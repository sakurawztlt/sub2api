...
func TestOpenAIGatewayServiceForwardRegistersCodexToolNameReverseMapping(t *testing.T) {
	gin.SetMode(gin.TestMode)
	upstreamSSE := strings.Join([]string{
		`data: {"type":"response.output_item.done","item":{"type":"function_call","name":"python__sub2api","call_id":"call_1","arguments":"{}"}}`,
		``,
		`data: {"type":"response.completed","response":{"id":"resp_tool_name","model":"gpt-5.6-sol","output":[],"usage":{"input_tokens":1,"output_tokens":1,"total_tokens":2}}}`,
		``,
		`data: [DONE]`,
		``,
	}, "\n")
	...
	body := []byte(`{"model":"gpt-5.6-sol","stream":true,"input":"run it","tools":[{"type":"function","name":"python","description":"run python","parameters":{"type":"object","properties":{}}}]}`)
	...
	require.NotContains(t, recorder.Body.String(), codexPythonToolAlias)
}

func TestOpenAIGatewayServiceForwardRejectsCodexToolNameCollisionWithoutAliasLeak(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	body := []byte(`{"model":"gpt-5.6-sol","stream":false,"input":"run it","tools":[{"type":"function","name":"python"},{"type":"function","name":"python__sub2api"}]}`)
	...
	require.NotContains(t, recorder.Body.String(), codexPythonToolAlias)
	require.Contains(t, recorder.Body.String(), "Conflicting tool names")
}

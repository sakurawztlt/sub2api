...
// Keep deferred video billing separate from the hot-path GatewayCache
// contract so existing cache doubles and non-video deployments remain source
// compatible while the production Redis cache can opt into durable billing.
...
func (s *OpenAIGatewayService) grokVideoBillingCache() (grokVideoBillingCache, error) {
	if s == nil || s.cache == nil {
		return nil, fmt.Errorf("grok video billing cache is unavailable")
	}
	cache, ok := s.cache.(grokVideoBillingCache)
	if !ok {
		return nil, fmt.Errorf("grok video billing cache is unsupported")
	}
	return cache, nil
}
...
func (s *OpenAIGatewayService) StoreGrokVideoPendingBilling(
	...
	cache, err := s.grokVideoBillingCache()
	if err != nil {
		return err
	}

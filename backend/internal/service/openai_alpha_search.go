...
// evolving alpha request or response schema. A non-nil result means a real 2xx
// search completed and must be recorded for per-call billing.
...
func (s *OpenAIGatewayService) buildOpenAIAlphaSearchResponsesWebSearchRequest(ctx context.Context, c *gin.Context, account *Account, alphaBody, body []byte, token string) (*http.Request, error) {
...
func stripOpenAIAlphaSearchResponsesHeaders(headers http.Header) {
	...
	for _, key := range []string{
		"OpenAI-Beta", "Session_ID", "Conversation_ID", "X-Codex-Beta-Features",
		"X-Codex-Turn-State", responsesLiteHeaderKey,
	} {
		headers.Del(key)
	}
}

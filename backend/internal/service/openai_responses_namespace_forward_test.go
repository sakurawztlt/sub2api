...
func TestOpenAIGatewayService_OAuthPreservesCodexNamespaceTools(t *testing.T) {
	...
	upstream := &httpUpstreamRecorder{responses: []*http.Response{
		newNamespaceForwardTestResponse(http.StatusOK, namespaceForwardOKResponse),
	}}
	c := newNamespaceForwardTestContext(body)

	result, err := newNamespaceForwardTestService(upstream).Forward(
		context.Background(), c, newNamespaceForwardOAuthAccount(), body,
	)
...
func TestOpenAIGatewayService_OAuthFlattenFlagRestoresLegacyBehavior(t *testing.T) {
	...
	c := newNamespaceForwardTestContext(body)
	account := newNamespaceForwardOAuthAccount()
...
func TestOpenAIGatewayService_ForwardClearsStaleNamespaceNames(t *testing.T) {
	...
	_, err := newNamespaceForwardTestService(upstream).Forward(
		context.Background(), c, newNamespaceForwardOAuthAccount(), body,
	)
...
func newNamespaceForwardTestService(upstream *httpUpstreamRecorder) *OpenAIGatewayService {
...
func newNamespaceForwardTestContext(body []byte) *gin.Context {
...
func newNamespaceForwardOAuthAccount() *Account {
	return &Account{
		ID:          5108,
...
func newNamespaceForwardTestResponse(status int, body string) *http.Response {

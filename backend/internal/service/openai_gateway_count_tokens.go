...
// ForwardResponsesInputTokens handles native OpenAI
// POST /v1/responses/input_tokens without entering generation billing.
...
func writeOpenAIResponsesInputTokensFallback(c *gin.Context, account *Account, prepared *openAIInputTokensCountPrepared, statusCode int, reason string) {
	...
	c.JSON(http.StatusOK, gin.H{"object": "response.input_tokens", "input_tokens": estimated})
}
...
// request locally because Grok has no compatible counting endpoint.

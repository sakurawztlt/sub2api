...
// handler 在 Forward 返回后读取以触发风控记录、邮件与真实 token 用量计费。
...
// 据此丢弃普通 result 并返回该哨兵，使 handler 改用专用路径依上游真实 token
// 记录与计费，同时不 failover、不重复写响应。
...
// cyberPolicyClientMessage preserves the upstream explanation unless it would
// expose the protected OpenAI/Codex backend identity or a secret-bearing URL.
func cyberPolicyClientMessage(message string, payload []byte) string {
	message = sanitizeUpstreamErrorMessage(strings.TrimSpace(message))
	if message == "" || containsOpenAICompatSensitiveBackendTerm(message, payload) {
		return "Request blocked by upstream cyber-security policy"
	}
	return message
}

// cyberPolicyClientBody keeps the provider body byte-identical for ordinary
// cyber errors, while preserving the relay's identity-redaction invariant.
func cyberPolicyClientBody(message string, body []byte) []byte {
	if !containsOpenAICompatSensitiveBackendTerm(message, body) {
		return body
	}

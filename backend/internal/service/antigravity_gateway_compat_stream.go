...
const antigravityCompatTemporaryUnavailableMessage = "The service is temporarily unavailable. Please retry."
...
func (a *antigravityChatStreamAdapter) WriteError(writer *antigravityClientWriter, reason string) {
	_ = reason
	writer.Fprintf("data: {\"error\":{\"message\":%q,\"type\":\"server_error\"}}\n\n", antigravityCompatTemporaryUnavailableMessage)
}
...
func (a *antigravityResponsesStreamAdapter) WriteError(writer *antigravityClientWriter, reason string) {
	_ = reason
	if a.anthropicState != nil && a.anthropicState.CompletedSent {
		return
	}
	event := apicompat.FailedAnthropicResponsesStreamEvent(
		a.anthropicState,
		"server_error",
		antigravityCompatTemporaryUnavailableMessage,
	)
	a.emitResponseEvent(event, writer)
}
...
func (s *AntigravityGatewayService) handleAntigravityCompatStream(
	...
	firstMeaningfulTimeout := s.antigravityCompatFirstMeaningfulTimeout()
	firstMeaningfulTimer, firstMeaningfulCh := newAntigravityCompatTimer(firstMeaningfulTimeout)
	...
	for {
		...
		case <-firstMeaningfulCh:
			...
			_ = resp.Body.Close()
			logger.LegacyPrintf("service.antigravity_gateway", "First meaningful event timeout (%s)", prefix)
			return nil, antigravityCompatFirstMeaningfulTimeoutError()
...
func (s *AntigravityGatewayService) antigravityCompatFirstMeaningfulTimeout() time.Duration {
	if s.settingService == nil || s.settingService.cfg == nil {
		return 0
	}
	return time.Duration(s.settingService.cfg.Gateway.FirstMeaningfulEventTimeoutSeconds) * time.Second
}
...
func stopAntigravityCompatTimer(timer *time.Timer) {
	if timer == nil {
		return
	}
	if !timer.Stop() {
...
func antigravityCompatFirstMeaningfulTimeoutError() error {
	return &UpstreamFailoverError{
		StatusCode:  http.StatusBadGateway,
		BreakSticky: true,

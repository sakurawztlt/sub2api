...
const (
	...
	// claudeCodeSystemPromptExpansion mirrors the tool-independent portion of the
	// real Claude Code system prompt. Tool-specific instructions are intentionally
	// omitted so the proxy does not change the caller's requested behavior.
...
// openAIStreamEventIsTerminalWithType reuses an already extracted event type so
// response hot paths do not scan the same SSE JSON payload twice.
...
// ErrStickySessionNotFound abstracts cache-driver miss errors for sticky bindings.
...
// AudioUsage records the billable unit for Grok realtime, TTS, and STT calls.
...
type ForwardResult struct {
	...
	// RequestedReasoningEffort is client intent before mapping and policy.
	...
	SearchCount        int            // Grok-native search tool calls (per-1k billing)
...
// sseStreamErrorEventError preserves the raw Anthropic event:error payload so
// the forwarding layer can classify it without exposing unsanitized details.
...
// bindGatewayStickySessionDuringSelection preserves eager sticky binding for
// ordinary requests. Profit-controlled requests bind only after the terminal
// post-slot admission check so a vetoed candidate cannot replace a healthy
// existing binding.
...
// BindStickySessionAfterProfitAdmission records the admitted account without
// overwriting a different pre-existing binding while the profit gate is active.
...
// rewriteSystemForNonClaudeCode is the legacy two-block compatibility helper
// retained for direct callers and its focused tests. Production OAuth mimic
// paths use rewriteSystemForNonClaudeCodeWithPromptBlocks and the configurable
// three-block shape below.
// 它将非 Claude Code 客户端的 system prompt 迁移至 messages，
...
func rewriteSystemForNonClaudeCode(body []byte, system any) []byte {
	...
	// 保留 block 级 cache_control，避免非 Claude Code system 重写时把客户设置的
	// 缓存断点折叠进一个普通文本块。
...
func extractSystemInstructionBlocks(system any) []map[string]any {
	blocks := []map[string]any{{"type": "text", "text": "[System Instructions]"}}
	ccPromptTrimmed := strings.TrimSpace(claudeCodeSystemPrompt)
	appendText := func(text string, cacheControl any) {
		text = strings.TrimSpace(text)
		if text == "" || text == ccPromptTrimmed || hasClaudeCodePrefix(text) {
			return
		}
	...
	case []any:
		for _, item := range v {
			...
			appendText(text, m["cache_control"])
		}
	}
	return blocks
}
...
func rewriteSystemForNonClaudeCodeWithPromptBlocks(body []byte, system any, expansionPrompt string, blocksConfig string) []byte {
	...
	originalSystemBlocks := extractSystemInstructionBlocks(system)
...
func (s *GatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) (forwardResultOut *ForwardResult, _ error) {
	startTime := time.Now()
	if parsed == nil {
	...
	defer func() {
		...
		if forwardResultOut != nil {
			if tier := anthropicSpeedServiceTier(account, parsed.Speed, anthropicSpeedModel(parsed, forwardResultOut)); tier != nil {
				forwardResultOut.ServiceTier = tier
			}
		}
	}()
	...
	directClaudeCode := IsClaudeCodeClient(ctx) || isClaudeCodeClient(c.GetHeader("User-Agent"), parsed.MetadataUserID)
	proxiedClaudeCode := !directClaudeCode && parsed.MetadataUserID != "" && systemHasBillingAttributionBlock(body)
	preserveClaudeCodeBody := directClaudeCode || proxiedClaudeCode
	shouldRewriteClaudeCodeBody := account.IsOAuth() && !preserveClaudeCodeBody
	// A proxied real-CLI body must retain its cache layout, but the gateway may
	// have replaced its headers. Keep body preservation and wire disguise as
	// separate decisions so OAuth still receives our locked CLI header profile.
	shouldMimicClaudeCodeHeaders := account.IsOAuth() && !directClaudeCode
	...
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		...
		upstreamReq, wireBody, err := s.buildUpstreamRequest(upstreamCtx, c, account, body, token, tokenType, reqModel, reqStream, shouldMimicClaudeCodeHeaders)
		releaseUpstreamCtx()
		if err != nil {
			return nil, err
		}
		...
		if resp.StatusCode == 400 {
			...
			if readErr == nil {
				...
				if s.shouldRectifySignatureError(ctx, account, respBody, reqModel) {
					...
					retryReq, retryWireBody, buildErr := s.buildUpstreamRequest(retryCtx, c, account, filteredBody, token, tokenType, reqModel, reqStream, shouldMimicClaudeCodeHeaders)
					releaseRetryCtx()
					if buildErr == nil {
						...
						if retryErr == nil {
							...
							if retryReadErr == nil && retryResp.StatusCode == 400 && s.isSignatureErrorPattern(ctx, account, retryRespBody) {
								...
								if looksLikeToolSignatureError(msg2) && time.Since(retryStart) < maxRetryElapsed {
									...
									retryReq2, retryWireBody2, buildErr2 := s.buildUpstreamRequest(retryCtx2, c, account, filteredBody2, token, tokenType, reqModel, reqStream, shouldMimicClaudeCodeHeaders)
									releaseRetryCtx2()
									if buildErr2 == nil {
				...
				if isThinkingBudgetConstraintError(errMsg) && s.settingService.IsBudgetRectifierEnabled(ctx) {
					...
					if applied && time.Since(retryStart) < maxRetryElapsed {
						...
						budgetRetryReq, budgetWireBody, buildErr := s.buildUpstreamRequest(budgetRetryCtx, c, account, rectifiedBody, token, tokenType, reqModel, reqStream, shouldMimicClaudeCodeHeaders)
	...
	if reqStream {
		...
		streamResult, err := s.handleStreamingResponse(ctx, resp, c, account, startTime, originalModel, reqModel, shouldMimicClaudeCodeHeaders)
		if err != nil {
			var streamEventErr *sseStreamErrorEventError
			if errors.As(err, &streamEventErr) {
				body := []byte(streamEventErr.RawData)
				...
				logger.LegacyPrintf("service.gateway", "[Forward] SSE error event in stream: Account=%d(%s) RequestID=%s Body=%s",
					account.ID, account.Name, resp.Header.Get("x-request-id"), truncateString(streamEventErr.RawData, 1000))
				return nil, &UpstreamFailoverError{
					StatusCode:   semanticStatus,
					ResponseBody: body,
				}
...
// anthropicSpeedServiceTier normalizes a genuinely supported Anthropic
// speed=fast request to the shared billable service tier.
...
func (s *GatewayService) buildUpstreamRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token, tokenType, modelID string, reqStream bool, mimicClaudeCode bool) (*http.Request, []byte, error) {
	...
	// Claude OAuth wire profile is version-locked for both transformed and
	// direct/proxied Claude Code bodies. The body decision must not allow an
...
// vertexSupportedBetaTokens is the allowlist accepted by Vertex AI's
// Anthropic endpoint. Vertex rejects unknown beta tokens with HTTP 400, so
// new Claude Code client tokens must be withheld until Vertex supports them.
...
// filterVertexBetaTokens applies the configured beta-policy drop set before
// the Vertex allowlist and preserves the client's first-seen token order.
...
func (s *GatewayService) computeFinalCountTokensAnthropicBeta(
	...
	if tokenType == "oauth" {
		if mimicClaudeCode {
			// mimic 路径必须与 messages 一样锁定 beta profile。客户端或前置
			// 网关携带的旧 beta（例如 fine-grained-tool-streaming）不能泄到上游。
...
type RecordUsageInput struct {
	...
	PricingAt          time.Time          // token pricing instant; zero preserves record-time fallback
...
func resolveUsageBillingRequestID(ctx context.Context, upstreamRequestID string) string {
	// Durable per-call money event IDs must win over client/local context IDs so
	// standalone search, video, and audio calls cannot collapse under a reused id.
	if requestID := strings.TrimSpace(upstreamRequestID); requestID != "" && isForcedUsageBillingRequestID(requestID) {
		return requestID
	}
	if ctx != nil {
...
func (s *GatewayService) calculateRecordUsageCost(
	...
	// Token 计费；SearchCount 是叠加费，不替代 token 成本。
...
func (s *GatewayService) ForwardCountTokens(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) error {
	...
	directClaudeCodeCT := IsClaudeCodeClient(ctx) || isClaudeCodeClient(c.GetHeader("User-Agent"), parsed.MetadataUserID)
	proxiedClaudeCodeCT := !directClaudeCodeCT && parsed.MetadataUserID != "" && systemHasBillingAttributionBlock(body)
	preserveClaudeCodeBody := directClaudeCodeCT || proxiedClaudeCodeCT
	shouldRewriteClaudeCodeBody := account.IsOAuth() && !preserveClaudeCodeBody
	...
	// 构建上游请求
	upstreamReq, wireBody, err := s.buildCountTokensRequest(ctx, c, account, body, token, tokenType, reqModel, shouldMimicClaudeCodeHeaders)
	if err != nil {
	...
	if resp.StatusCode == 400 && s.shouldRectifySignatureError(ctx, account, respBody, reqModel) {
		...
		filteredBody := FilterThinkingBlocksForRetry(body, reqModel)
		retryReq, retryWireBody, buildErr := s.buildCountTokensRequest(ctx, c, account, filteredBody, token, tokenType, reqModel, shouldMimicClaudeCodeHeaders)
		if buildErr == nil {

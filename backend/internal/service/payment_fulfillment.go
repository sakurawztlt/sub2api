...
func (s *PaymentService) applyAffiliateRebateForOrder(ctx context.Context, o *dbent.PaymentOrder) error {
	...
	failRebateTx := func(operation string, cause error) error {
		// Release the transaction before writing the failure audit. In
		// particular SQLite otherwise waits on its own write lock forever;
		// the ordering is also safer for databases with row-level locks.
		_ = tx.Rollback()
	...
	if err != nil {
		return failRebateTx("claim affiliate rebate audit", err)
	}
	if !claimed {
		return nil
	}
	...
	if err != nil {
		return failRebateTx("accrue affiliate rebate", err)
	}
	...
	if rebateAmount <= 0 {
		if err := s.updateClaimedAffiliateRebateAudit(txCtx, tx.Client(), o.ID, "AFFILIATE_REBATE_SKIPPED", map[string]any{
			...
			return failRebateTx("update affiliate rebate skipped audit", err)
		}
		if err := tx.Commit(); err != nil {
			return failRebateTx("commit affiliate rebate tx", err)
		}
		return nil
	}
	...
	if err := s.updateClaimedAffiliateRebateAudit(txCtx, tx.Client(), o.ID, "AFFILIATE_REBATE_APPLIED", map[string]any{
		...
		return failRebateTx("update affiliate rebate applied audit", err)
	}

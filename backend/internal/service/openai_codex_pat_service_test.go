...
func TestOpenAIOAuthServiceValidateCodexPersonalAccessToken(t *testing.T) {
	...
	require.Equal(t, wantOriginator, originator)
	require.Equal(t, wantUserAgent, userAgent)
...
func TestOpenAIOAuthServiceValidateCodexPersonalAccessTokenRequiresATPrefix(t *testing.T) {
	svc := NewOpenAIOAuthService(nil, nil)
	defer svc.Stop()
...
func TestNormalizeOpenAIPersonalAccessTokenCredentialsRemovesOAuthFields(t *testing.T) {
	account := &Account{Platform: PlatformOpenAI, Type: AccountTypeOAuth, Credentials: map[string]any{"auth_mode": "personal_access_token"}}
	credentials := map[string]any{
		"access_token": "at-test-token", "refresh_token": "stale", "id_token": "stale",
		"expires_at": "2026-01-01T00:00:00Z", "expires_in": 3600, "client_id": "stale-client",
		"model_mapping": map[string]any{"gpt-5": "gpt-5-codex"}, "chatgpt_account_is_fedramp": true,
	}

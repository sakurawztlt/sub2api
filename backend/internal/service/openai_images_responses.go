...
// extractOpenAIImagesModelText collects terminal text emitted instead of an
// image. It supports both JSON terminal objects and split SSE deltas.
func extractOpenAIImagesModelText(body []byte) string {
	var collected strings.Builder
	collect := func(text string) {
		if text = strings.TrimSpace(text); text != "" {
			if collected.Len() > 0 {
				_ = collected.WriteByte(' ')
			}
...
func summarizeOpenAIImagesNoOutputBody(body []byte) string {
	...
	if lastType != "" {
		fmt.Fprintf(&summary, " last_event=%s", lastType)
	}
	if status != "" {
		fmt.Fprintf(&summary, " status=%s", status)
	}
	if incompleteReason != "" {
		fmt.Fprintf(&summary, " incomplete_reason=%s", incompleteReason)
	}
	...
	if snippet != "" {
		fmt.Fprintf(&summary, " body=%s", snippet)
	}
...
func extractOpenAIImagesUpstreamError(body []byte) *OpenAIImagesUpstreamError {
	for _, line := range bytes.Split(body, []byte("\n")) {
		...
		if upstreamErr := openAIImagesUpstreamErrorFromSSEPayload([]byte(data)); upstreamErr != nil {
			return upstreamErr
		}
	}
	if gjson.ValidBytes(body) {
		return openAIImagesUpstreamErrorFromSSEPayload(body)
	}
	return nil
}
...
func openAIImagesUpstreamErrorFromSSEPayload(payload []byte) *OpenAIImagesUpstreamError {
	...
	if errObj := gjson.GetBytes(payload, "error"); errObj.Exists() {
		return openAIImagesUpstreamErrorFromGJSON(errObj, "")
	}
	if responseErr := gjson.GetBytes(payload, "response.error"); responseErr.Exists() {
		return openAIImagesUpstreamErrorFromGJSON(responseErr, gjson.GetBytes(payload, "response.id").String())
	}
	return nil
}
...
func (s *OpenAIGatewayService) handleOpenAIImagesOAuthStreamingResponse(
	...
	processData := func(dataBytes []byte) {
		...
		case "response.completed":
			...
			if len(finalResults) == 0 {
				...
				processDataErr = &UpstreamFailoverError{
					StatusCode:             http.StatusBadGateway,
	...
	finalizePending := func() error {
		...
		if textFallbackErr := openAIImagesTextFallbackErrorForText(fallbackText.String()); textFallbackErr != nil {
			retryable := IsOpenAIImagesRetryableUpstreamError(textFallbackErr)
			if !clientDisconnected && (!retryable || OpenAIImagesJSONKeepaliveAdjustedWrittenSize(c) != writerSizeBeforeResponse) {
				s.tryWriteOpenAIImagesStreamEvent(c, flusher, &clientDisconnected, &lastDownstreamWriteAt, "error", buildOpenAIImagesStreamErrorBodyFromUpstream(textFallbackErr))
			}
			setOpsUpstreamError(c, textFallbackErr.clientStatusCode(), textFallbackErr.clientMessage(), summarizeOpenAIImagesNoOutputBody(nil))
			return textFallbackErr
		}

		setOpsUpstreamError(c, http.StatusBadGateway, "stream disconnected before image generation completed", summarizeOpenAIImagesNoOutputBody(nil))
...
func (s *OpenAIGatewayService) forwardOpenAIImagesOAuth(
	...
	// Exclude non-streaming JSON keepalive bytes so an earlier failover attempt
	// cannot make a later attempt look as if it already emitted image output.
	writerSizeBeforeResponse := OpenAIImagesJSONKeepaliveAdjustedWrittenSize(c)
	if parsed.Stream {

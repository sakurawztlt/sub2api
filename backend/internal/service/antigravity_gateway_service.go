...
func (s *AntigravityGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte, isStickySession bool) (forwardResultOut *ForwardResult, _ error) {
...
func (s *AntigravityGatewayService) ForwardGemini(ctx context.Context, c *gin.Context, account *Account, originalModel string, action string, stream bool, body []byte, isStickySession bool, options ...ForwardGeminiOption) (forwardResultOut *ForwardResult, _ error) {
	beginUpstreamResponseModelObservation(c)
	defer func() {
		forwardResultOut = attachObservedUpstreamResponseModel(c, forwardResultOut)
	}()
...
func (s *AntigravityGatewayService) collectClaudeStreamResponse(resp *http.Response, startTime time.Time, originalModel string, contexts ...*gin.Context) ([]byte, *antigravityStreamResult, error) {
	var responseObserver *upstreamResponseModelObserver
	if len(contexts) > 0 && contexts[0] != nil {
		responseObserver = upstreamResponseModelObserverFromContext(contexts[0])
		if responseObserver == nil {
			responseObserver = beginUpstreamResponseModelObservation(contexts[0])
		}
	} else {
		responseObserver = &upstreamResponseModelObserver{}
	}
	scanner := bufio.NewScanner(resp.Body)
	maxLineSize := defaultMaxLineSize
	...
	// A stream that continuously emits metadata, malformed frames, or usage
	// heartbeats must not keep a buffered request alive forever. The first
	// meaningful timer is independent from data arrival; the total timer is a
	// hard cap and synthesizes the partial response when usable content exists.
	var bufferedTotalTimeout, bufferedFirstMeaningfulTimeout time.Duration
	if s.settingService != nil && s.settingService.cfg != nil {
		gatewayCfg := s.settingService.cfg.Gateway
		if gatewayCfg.BufferedTotalTimeout > 0 {
			bufferedTotalTimeout = time.Duration(gatewayCfg.BufferedTotalTimeout) * time.Second
		}
		if gatewayCfg.BufferedFirstMeaningfulTimeout > 0 {
			bufferedFirstMeaningfulTimeout = time.Duration(gatewayCfg.BufferedFirstMeaningfulTimeout) * time.Second
		}
	}
	totalTimer, totalCh := newAntigravityCompatTimer(bufferedTotalTimeout)
	if totalTimer != nil {
		defer totalTimer.Stop()
	}
	firstMeaningfulTimer, firstMeaningfulCh := newAntigravityCompatTimer(bufferedFirstMeaningfulTimeout)
	...
	for {
		...
		case ev, ok := <-events:
			...
			responseObserver.ObserveGemini(inner)
		...
		case <-firstMeaningfulCh:
			if meaningfulResponse {
				firstMeaningfulCh = nil
				continue
			}
			_ = resp.Body.Close()
			logger.LegacyPrintf("service.antigravity_gateway", "Buffered first meaningful event timeout (antigravity claude non-stream)")
			return nil, nil, &UpstreamFailoverError{
				StatusCode:  http.StatusBadGateway,
				BreakSticky: true,
				Reason:      "buffered_first_meaningful_timeout",
			}

		case <-totalCh:
			_ = resp.Body.Close()
			if meaningfulResponse {
				logger.LegacyPrintf("service.antigravity_gateway", "Buffered total timeout with meaningful response (antigravity claude non-stream)")
				goto returnResponse
			}
			logger.LegacyPrintf("service.antigravity_gateway", "Buffered total timeout without meaningful response (antigravity claude non-stream)")
			return nil, nil, &UpstreamFailoverError{
				StatusCode:  http.StatusBadGateway,
				BreakSticky: true,
...
returnResponse:
	...
	if !meaningfulResponse {
		...
		return nil, nil, &UpstreamFailoverError{
			StatusCode:             http.StatusBadGateway,
			ResponseBody:           []byte(`{"error":{"type":"api_error","message":"The service is temporarily unavailable. Please retry."}}`),
			RetryableOnSameAccount: true,
		}
	}
...
// handleClaudeStreamToNonStreaming collects an upstream stream and writes the
// resulting Claude response for the native Messages path.
func (s *AntigravityGatewayService) handleClaudeStreamToNonStreaming(c *gin.Context, resp *http.Response, startTime time.Time, originalModel string) (*antigravityStreamResult, error) {
	claudeResp, streamRes, err := s.collectClaudeStreamResponse(resp, startTime, originalModel, c)
	if err != nil {
...
func (s *AntigravityGatewayService) ForwardUpstream(ctx context.Context, c *gin.Context, account *Account, body []byte) (forwardResultOut *ForwardResult, _ error) {
	beginUpstreamResponseModelObservation(c)
	defer func() {
		forwardResultOut = attachObservedUpstreamResponseModel(c, forwardResultOut)
	}()

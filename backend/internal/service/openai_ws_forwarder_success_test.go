...
func TestOpenAIGatewayService_Forward_WSv2_OAuthStoreFalseByDefault(t *testing.T) {
	...
	// OAuth 账号的 session_id/conversation_id 分别隔离，同时保留两者的语义。
...
func TestOpenAIGatewayService_Forward_WSv2_OAuthKeepsConversationIDDistinct(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	require.Equal(t, isolateOpenAIUpstreamSessionID(0, account, "conv-fresh"), captureDialer.lastHeaders.Get("conversation_id"))
	require.Equal(t, isolateOpenAIUpstreamSessionID(0, account, "stable-session"), captureDialer.lastHeaders.Get("session_id"), "upstream session_id should preserve the client's session boundary")
}

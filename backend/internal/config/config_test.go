...
func TestLoadFromExplicitConfigFile(t *testing.T) {
	resetViperWithJWTSecret(t)
	configFile := filepath.Join(t.TempDir(), "relay-config.yaml")
...
func TestExplicitConfigFileTakesPrecedenceOverDataDir(t *testing.T) {
	resetViperWithJWTSecret(t)
	dataDir := t.TempDir()
...
func TestLoadDefaultSecurityToggles(t *testing.T) {
	...
	wantHosts := []string{"api.kimi.com", "api.moonshot.ai", "api.moonshot.cn"}
...
func TestValidateConfig_OpenAIWSRules(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "scheduler_score_weights 基础权重总和不能溢出",
			mutate: func(c *Config) {
	...
	t.Run("upstream_cost 可作为唯一有效基础调度权重", func(t *testing.T) {
		cfg := buildValid(t)

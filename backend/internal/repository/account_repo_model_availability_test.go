...
type modelAvailabilityQueryMatcher struct {
	actual *string
}

func (m modelAvailabilityQueryMatcher) Match(_, actual string) error {
	if m.actual == nil {
...
func TestListModelAvailabilityCandidates_GroupQueryIgnoresTransientState(t *testing.T) {
	...
	db, mock, err := sqlmock.New(sqlmock.QueryMatcherOption(modelAvailabilityQueryMatcher{actual: &capturedSQL}))

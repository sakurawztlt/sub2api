...
func (r *channelRepository) ListModelPricing(ctx context.Context, channelID int64) ([]service.ChannelModelPricing, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, channel_id, platform, models, billing_mode, input_price, image_input_price, output_price, cache_write_price, cache_read_price, image_output_price, per_request_price, created_at, updated_at
...
func (r *channelRepository) UpdateModelPricing(ctx context.Context, pricing *service.ChannelModelPricing) error {
	...
	result, err := r.db.ExecContext(ctx,
		`UPDATE channel_model_pricing
		 SET models = $1, billing_mode = $2, input_price = $3, image_input_price = $4, output_price = $5, cache_write_price = $6, cache_read_price = $7, image_output_price = $8, per_request_price = $9, platform = $10, updated_at = NOW()
		 WHERE id = $11`,
		modelsJSON, billingMode, pricing.InputPrice, pricing.ImageInputPrice, pricing.OutputPrice, pricing.CacheWritePrice, pricing.CacheReadPrice,
...
func createModelPricingExec(ctx context.Context, exec dbExec, pricing *service.ChannelModelPricing) error {
	...
	err = exec.QueryRowContext(ctx,
		`INSERT INTO channel_model_pricing (channel_id, platform, models, billing_mode, input_price, image_input_price, output_price, cache_write_price, cache_read_price, image_output_price, per_request_price)

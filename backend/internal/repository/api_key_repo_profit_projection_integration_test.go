...
func TestGetByKeyForAuthCarriesProfitControlProjection(t *testing.T) {
	...
	group := mustCreateGroup(t, integrationEntClient, &service.Group{
		...
		VideoModelPrices:          map[string]map[string]float64{"sora-2": {"720p": 0.17}},
		SearchPricePer1k:          &searchPrice,
		AudioRealtimePricePerMin:  &audioPrice,
		LongContextPricingEnabled: true,
		ModelPricing: []service.ChannelModelPricing{{
			Models: []string{"gpt-5.4-*"}, BillingMode: service.BillingModeToken, InputPrice: &modelInputPrice,
		}},
	...
	require.InDelta(t, 1.4, got.Group.ImageRateMultiplier, 1e-9)
	require.Equal(t, &imagePrice, got.Group.ImagePrice1K)
	require.Equal(t, map[string]map[string]float64{"sora-2": {"720p": 0.17}}, got.Group.VideoModelPrices)
	require.Equal(t, &searchPrice, got.Group.SearchPricePer1k)
	require.Equal(t, &audioPrice, got.Group.AudioRealtimePricePerMin)
	require.True(t, got.Group.LongContextPricingEnabled)
	require.Len(t, got.Group.ModelPricing, 1)
	require.Equal(t, []string{"gpt-5.4-*"}, got.Group.ModelPricing[0].Models)
	require.Equal(t, &modelInputPrice, got.Group.ModelPricing[0].InputPrice)
}

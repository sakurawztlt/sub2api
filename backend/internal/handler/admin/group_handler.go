...
type groupDuplicateAdminService interface {
	DuplicateGroup(ctx context.Context, id int64, actorScope, operationKey string) (*service.Group, error)
	RecoverDuplicateGroup(ctx context.Context, id int64, actorScope, operationKey string) (*service.Group, error)
}
...
func (h *GroupHandler) Create(c *gin.Context) {
	...
	if err := service.ValidateProfitControlConfig(service.NormalizeGroupPlatform(req.Platform), req.ProfitControlEnabled, derefFloat64Default(req.ProfitMinMargin, 0), derefFloat64Default(req.ProfitSafetyBuffer, 0)); err != nil {
		response.BadRequest(c, err.Error())
		return
	}
...
func (h *GroupHandler) Duplicate(c *gin.Context) {
	...
	duplicateService, ok := h.adminService.(groupDuplicateAdminService)
	if !ok {
		response.InternalError(c, "Group duplication is unavailable")
		return
	}
	...
	result, err := executeAdminIdempotent(
		...
		func(ctx context.Context) (any, error) {
			group, execErr := duplicateService.DuplicateGroup(ctx, groupID, actorScope, c.GetHeader("Idempotency-Key"))
			if execErr != nil {
				return nil, execErr
			}
	...
	if err != nil {
		...
		if reason == infraerrors.Reason(service.ErrIdempotencyInProgress) || reason == infraerrors.Reason(service.ErrIdempotencyStoreUnavail) {
			recovered, recoverErr := duplicateService.RecoverDuplicateGroup(c.Request.Context(), groupID, actorScope, c.GetHeader("Idempotency-Key"))
			if recoverErr != nil {

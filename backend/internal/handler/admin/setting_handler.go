...
// SetStepUpDeps attaches the services used to validate changes to the step-up
// security switch while preserving the constructor used by existing tests.
...
// setting key they persist to. Every other value-typed field uses its JSON name.
var settingKeyJSONAliases = map[string]string{
	"smtp_from_email": service.SettingKeySMTPFrom,
}

// settingKeyByJSONName maps value-typed request fields to their setting keys.
// Pointer fields already preserve omitted values in UpdateSettings and remain
// excluded so their existing fail-closed normalization is unchanged.
...
// omittedSettingKeys reports the setting keys the incoming payload never
// carried, allowing partial API clients to preserve stored values.
...
func (h *SettingHandler) UpdateSettings(c *gin.Context) {
	...
	settings := &service.SystemSettings{
		...
		RegistrationEmailDomainQuotaEnabled: func() bool {
			if req.RegistrationEmailDomainQuotaEnabled != nil {
				return *req.RegistrationEmailDomainQuotaEnabled
			}
			return previousSettings.RegistrationEmailDomainQuotaEnabled
		}(),
		...
		DefaultProxyID: func() *int64 {
			if _, sent := sentFields[service.SettingKeyDefaultProxyID]; !sent {
				return previousSettings.DefaultProxyID
			}
			if req.DefaultProxyID == nil || *req.DefaultProxyID <= 0 {
				return nil
			}
		...
		ChannelMonitorMode: func() string {
			if req.ChannelMonitorMode != nil {
				return strings.TrimSpace(*req.ChannelMonitorMode)
			}
...
func diffSettings(before *service.SystemSettings, after *service.SystemSettings, beforeAuthSourceDefaults *service.AuthSourceDefaultSettings, afterAuthSourceDefaults *service.AuthSourceDefaultSettings, req UpdateSettingsRequest) []string {
	...
	if before.RegistrationEmailDomainQuotaEnabled != after.RegistrationEmailDomainQuotaEnabled {
		changed = append(changed, service.SettingKeyRegistrationEmailDomainQuotaEnabled)
	}
	...
	if before.CompactHomeEnabled != after.CompactHomeEnabled {
		changed = append(changed, service.SettingKeyCompactHomeEnabled)
	}
	if before.HideCcsImportButton != after.HideCcsImportButton {
		changed = append(changed, "hide_ccs_import_button")
	}
	if !equalInt64Ptr(before.DefaultProxyID, after.DefaultProxyID) {
		changed = append(changed, service.SettingKeyDefaultProxyID)
	}
	...
	if before.ChannelMonitorMode != after.ChannelMonitorMode {
		changed = append(changed, service.SettingKeyChannelMonitorMode)
	}
	if before.ChannelMonitorDefaultIntervalSeconds != after.ChannelMonitorDefaultIntervalSeconds {
		changed = append(changed, "channel_monitor_default_interval_seconds")
	}
	if before.ChannelMonitorHideThroughput != after.ChannelMonitorHideThroughput {
		changed = append(changed, service.SettingKeyChannelMonitorHideThroughput)
	}

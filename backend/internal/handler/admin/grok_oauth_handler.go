...
// CreateAccountsFromSSO converts Grok Web SSO cookies to Build OAuth tokens
// and persists only the converted OAuth credentials. The raw SSO values are
// neither returned nor copied into account credentials.
func (h *GrokOAuthHandler) CreateAccountsFromSSO(c *gin.Context) {
	...
	if len(tokens) > grokSSOImportMaxItems {
		response.BadRequest(c, "sso_tokens exceeds maximum batch size")
		return
	}
	...
	for i, token := range tokens {
		...
		select {
		case jobs <- grokSSOImportJob{index: i, token: token}:
		case <-ctx.Done():
			dispatched = false
		}
		if !dispatched {
...
func (h *GrokOAuthHandler) safeCreateAccountFromSSOToken(ctx context.Context, req GrokSSOToOAuthRequest, token string, index, total int) (result grokSSOImportWorkerResult) {
	defer func() {
		if recovered := recover(); recovered != nil {
			slog.Error("grok_sso_import_worker_panic", "index", index, "recovery_type", panicType(recovered))
			result = grokSSOImportWorkerResult{
				item: GrokSSOToOAuthItemResult{Index: index, Error: "internal worker failure"},
			}
		}
	}()
...
// grokSSOImportCredentials accepts only operator configuration from the
// request. OAuth token fields always come from BuildAccountCredentials.
func grokSSOImportCredentials(built map[string]any, reqCredentials map[string]any) map[string]any {
	...
	for key, value := range reqCredentials {
		if _, ok := allowedReqKeys[key]; !ok || service.IsSensitiveCredentialKey(key) {
			continue
		}
		ops[key] = value
	}
...
func isGrokSSOSecretKey(key string) bool {
	if service.IsSensitiveCredentialKey(key) {
		return true
	}
	switch strings.ToLower(strings.TrimSpace(key)) {
	case "access_token", "refresh_token", "id_token", "api_key", "session_key", "cookie",
...
func grokSSOImportErrorMessage(err error) string {
	...
	safeMessages := map[string]string{
		"GROK_OAUTH_NO_SSO_TOKEN":           "Grok SSO token is invalid",
		"GROK_OAUTH_CLIENT_NOT_CONFIGURED":  "Grok OAuth service is unavailable",
		"GROK_OAUTH_PROXY_NOT_AVAILABLE":    "selected proxy is unavailable",
		"GROK_OAUTH_PROXY_NOT_FOUND":        "selected proxy was not found",
		"GROK_OAUTH_PROXY_LOOKUP_FAILED":    "proxy lookup is temporarily unavailable",
		"GROK_OAUTH_INVALID_TOKEN_RESPONSE": "Grok SSO conversion returned an invalid token response",
	}
	if message, ok := safeMessages[status.Reason]; ok {

...
func estimateOpsErrorLogJobBytes(entry *service.OpsInsertErrorLogInput) int64 {
	...
	size := fixedOverhead + len(entry.RequestID) + len(entry.ClientRequestID) +
		...
		len(entry.ErrorOwner) + len(entry.APIKeyPrefix) + len(entry.AttemptedKeyPrefix)
	if entry.RequestHeadersJSON != nil {
		size += len(*entry.RequestHeadersJSON)
	}
	if entry.RequestBodyJSON != nil {
		size += len(*entry.RequestBodyJSON)
	}
...
func setOpsRequestContext(c *gin.Context, model string, stream bool, requestBodies ...[]byte) {
	if c == nil {
		return
	}
	...
	if len(requestBodies) > 0 && len(requestBodies[0]) > 0 {
		c.Set(opsRequestBodyKey, requestBodies[0])
	}
...
func logOpsStreamErrorValue(c *gin.Context, ops *service.OpsService, wireStatus int, streamErr service.OpsStreamError) {
	...
	if streamErr.CountTowardsSLA && streamErr.IntendedStatus >= http.StatusBadRequest {

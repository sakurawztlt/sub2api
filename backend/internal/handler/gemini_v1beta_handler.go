...
func (h *GatewayHandler) GeminiV1BetaGetModel(c *gin.Context) {
	...
	if !service.IsSafeGeminiModelPathSegment(modelName) {
		googleError(c, http.StatusBadRequest, "Invalid mapped model")
		return
	}
	...
	for {
		...
		if err != nil {
			...
			if errors.As(err, &failoverErr) {
				failoverAction := fs.HandleFailoverError(c.Request.Context(), h.gatewayService, account.ID, account.Platform, failoverErr, effectiveSameAccountRetryLimit(failoverErr, account))

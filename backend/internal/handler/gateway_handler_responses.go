	...
	for {
		...
		if shouldUseAntigravityCompat(account) {
			if h.antigravityGatewayService == nil {
				h.responsesErrorResponse(c, http.StatusBadGateway, "server_error", anthropicTemporaryUnavailableMessage)
				if accountReleaseFunc != nil {
					accountReleaseFunc()
				}
				return
		...
		if err != nil {
			...
			if errors.As(err, &failoverErr) {
				...
				action := fs.HandleFailoverError(requestCtx, h.gatewayService, account.ID, account.Platform, failoverErr, effectiveSameAccountRetryLimit(failoverErr, account))
				switch action {
				case FailoverContinue:
					continue

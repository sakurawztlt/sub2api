...
func TestHandleFailoverError_BasicSwitch(t *testing.T) {
	t.Run("显式停止不切换账号且旧错误默认仍切换", func(t *testing.T) {
		...
		action := fs.HandleFailoverError(context.Background(), mock, 100, service.PlatformGrok, stopErr)
		...
		action = fs.HandleFailoverError(context.Background(), mock, 100, service.PlatformGrok, legacyErr)
	...
	t.Run("已取消的认证失败不改变切换状态", func(t *testing.T) {
		...
		action := fs.HandleFailoverError(ctx, mock, 101, service.PlatformGrok, err)

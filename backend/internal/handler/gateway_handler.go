		...
		for {
			...
			if err != nil {
				...
				if errors.As(err, &failoverErr) {
					...
					action := fs.HandleFailoverError(c.Request.Context(), h.gatewayService, account.ID, account.Platform, failoverErr, effectiveSameAccountRetryLimit(failoverErr, account))
					switch action {
					case FailoverContinue:
						continue
			...
			case messagesForwardRouteOpenAICompat:
				...
				if currentAPIKey.Group != nil {
					...
					if defaultMappedModel == "" {
						defaultMappedModel = strings.TrimSpace(currentAPIKey.Group.ResolveMessagesDispatchModel(attemptParsedReq.Model))
					}
				}
...
func (h *GatewayHandler) Models(c *gin.Context) {
	...
	// Fallback to default models for the selected platform.
...
func sendMockInterceptStream(c *gin.Context, model string, interceptType InterceptType) {
	...
	// Keep the message_start field order and usage shape aligned with Anthropic.

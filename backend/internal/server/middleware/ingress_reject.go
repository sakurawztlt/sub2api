...
// IngressRejectReason identifies expected gateway admission failures that
// should not be recorded as operational upstream errors.
...
func invalidAuthClientKey(c *gin.Context) string {
	return normalizeIngressRejectIP(ip.GetClientIP(c))
}
...
func recordIngressReject(c *gin.Context, reason IngressRejectReason) {
	...
	clientIP := normalizeIngressRejectIP(ip.GetClientIP(c))

...
func TestAPIKeyAuthForwardsUserScopedOpenAIFastPolicyToUpstream(t *testing.T) {
	...
	upstreamBodies := make(chan []byte, 3)
	...
	send := func(apiKey, serviceTier string) {
		request := httptest.NewRequest(
			http.MethodPost,
			"/v1/responses",
			bytes.NewBufferString(`{"model":"gpt-5","stream":false,"service_tier":"`+serviceTier+`","input":"hi"}`),
		)
	...
	send("key-user-42", "priority")
	send("key-user-43", "priority")
	send("key-user-42", "flex")

	allowedUserBody := <-upstreamBodies
	otherUserBody := <-upstreamBodies
	forcedUserBody := <-upstreamBodies
	...
	require.Equal(t, service.OpenAIFastTierPriority, gjson.GetBytes(forcedUserBody, "service_tier").String())
}

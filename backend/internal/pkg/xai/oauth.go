...
var (
	...
	// *.api.x.ai covers the official regional endpoints operators can pin for
	// inference failover (for example us-west-2.api.x.ai).
...
// ValidateTrustedBaseURL only accepts the official xAI API/CLI hosts. OAuth
// callers use this validator so an unsafe custom address cannot receive a
// bearer token unless the service-layer operator policy explicitly permits it.
...
func BuildImagesGenerationsURLWithValidator(baseURL string, validator BaseURLValidator) (string, error) {
	return buildAPIURLWithValidator(baseURL, "/images/generations", validator)
}
...
func BuildImagesEditsURLWithValidator(baseURL string, validator BaseURLValidator) (string, error) {
	return buildAPIURLWithValidator(baseURL, "/images/edits", validator)
}

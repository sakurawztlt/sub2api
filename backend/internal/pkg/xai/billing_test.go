...
func TestBuildBillingURLWithValidator(t *testing.T) {
	...
	allowTestRelay := func(raw string) (string, error) { return raw, nil }
	monthlyURL, err := BuildBillingURLWithValidator("https://relay.example.test/v1", false, allowTestRelay)
	require.NoError(t, err)

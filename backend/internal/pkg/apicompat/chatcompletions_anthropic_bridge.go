...
func anthropicUserToChatMessages(raw json.RawMessage) ([]ChatMessage, error) {
	...
	for _, b := range blocks {
		...
		out = append(out, ChatMessage{
			Role:       "tool",
			Content:    content,
			ToolCallID: toResponsesCallID(b.ToolUseID),
		})
	...
	if hasImage && multimodalCueEnabled() {
		parts = append(parts, ChatContentPart{Type: "text", Text: multimodalCueText})
	}
...
func ChatCompletionsResponseToAnthropic(resp *ChatCompletionsResponse, model string) *AnthropicResponse {
	...
	if resp != nil {
		...
		if resp.Usage != nil {
			out.Usage = chatUsageToAnthropicUsage(resp.Usage, model)
		}
	}
...
func chatUsageToAnthropicUsage(usage *ChatUsage, model string) AnthropicUsage {
	if usage == nil {
	...
	inputTokens, cacheCreationTokens, cachedTokens := estimateAnthropicCacheUsageWithExplicitCreation(
		usage.PromptTokens,
		cachedTokens,
		cacheCreationTokens,
		model,
		AnthropicUsageEstimationOptions{},
	)
...
func ChatCompletionsChunkToAnthropicEvents(
	...
	if chunk.Usage != nil {
		u := chatUsageToAnthropicUsage(chunk.Usage, state.Model)

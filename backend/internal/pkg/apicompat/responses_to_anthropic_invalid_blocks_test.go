...
func TestResponsesToAnthropic_UserMessageInputFileKeepsSafePlaceholder(t *testing.T) {
	...
	requireAnthropicMessagesAreSendable(t, messages)
	require.Len(t, messages, 1)
	require.Contains(t, string(messages[0].Content), "[File reference: file_1]")
	require.NotContains(t, string(messages[0].Content), `"type":"input_file"`)
}
...
// 同时保留本地 call_* -> toolu_* 的 Anthropic ID 伪装。
func TestResponsesToAnthropic_CodexToolRoundStaysIntactAndSendable(t *testing.T) {
	...
	expectedToolUseID := fromResponsesCallIDToAnthropic("call_1")
	for _, m := range messages {
		for _, b := range parseContentBlocks(m.Content) {
			switch b.Type {
			case "tool_use":
				sawToolUse = true
				require.Equal(t, expectedToolUseID, b.ID)
				require.Equal(t, "shell", b.Name)
			case "tool_result":
				sawToolResult = true
				require.Equal(t, expectedToolUseID, b.ToolUseID)
			}
		}
	}

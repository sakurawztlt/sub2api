...
// The TrafficCaptureFunc type is an adapter to allow the use of ordinary function as a Querier.
type TrafficCaptureFunc func(context.Context, *ent.TrafficCaptureQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f TrafficCaptureFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.TrafficCaptureQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.TrafficCaptureQuery", q)
}

// The TraverseTrafficCapture type is an adapter to allow the use of ordinary function as Traverser.
type TraverseTrafficCapture func(context.Context, *ent.TrafficCaptureQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseTrafficCapture) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseTrafficCapture) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.TrafficCaptureQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.TrafficCaptureQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.TrafficCaptureQuery:
		return &query[*ent.TrafficCaptureQuery, predicate.TrafficCapture, trafficcapture.OrderOption]{typ: ent.TypeTrafficCapture, tq: q}, nil
